...
// GET finds and displays recommended articles
router.get(
	'/recommended',
	authenticateLogin,
	asyncHandler(async (req, res) => {
	...
	asyncHandler(async (req, res) => {
		...
		const articles = await Article.findAll({
			...
			include: [
				{
					model: User,
					attributes: ['firstName', 'lastName', 'emailAddress', 'accessLevel'],
				},
			],
			where: { userId: id },
			limit: articleLimit,
...
router.put(
	...
	asyncHandler(async (req, res) => {
		...
		if (isOwner || isAdmin) {
			await Article.update(req.body, { where: { id: req.query.id } }).then(
				(response) => {
					if (!response.name) {
						res.status(204).end()
					}
			...
			await Article.update(
				...
				if (!response.name) {
					...
					if (isAccrediting && !alreadyAccredited) {
						...
						updatedAccreditedArticles = accreditedArticles.filter(
							(id) => id !== article.id
						)
					} else if (!isAccrediting && alreadyAccredited) {
						updatedAccreditedArticles = accreditedArticles.filter(
					...
					if (!isAccrediting && !alreadyDiscredited) {
						...
						updatedDiscreditedArticles = discreditedArticles.filter(
							(id) => id !== article.id
						)
					} else if (isAccrediting && alreadyDiscredited) {
						updatedDiscreditedArticles = discreditedArticles.filter(
...
router.delete(
	...
	asyncHandler(async (req, res) => {
		...
		if (isOwner || isAdmin) {
			await Article.destroy({ where: { id: id } }).then(async () => {
				await User.update(
