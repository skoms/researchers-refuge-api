...
router.get(
	...
	asyncHandler(async (req, res) => {
		...
		const testedQuery = /\s/g.test(query)
			? `(${query.split(' ').join(')|(')})`
			: query
		const users = await User.findAll({
			attributes: {
...
router.put(
	...
	asyncHandler(async (req, res) => {
		...
		if (isOwner || isAdmin) {
			await User.update(updatedData, { where: { id: id } }).then((response) => {
				if (!response.name) {
					res.status(204).end()
				}
			})
...
router.put(
	...
	asyncHandler(async (req, res) => {
		...
		if (isOnline && user.id !== target.id) {
			...
			const targetId =
				typeof target.id === 'number' ? target.id : parseInt(target.id)

			const following = user.following
			const followers = target.followers

			const updatedFollowing = !following.includes(targetId)
				? [...following, targetId]
